...
from on_secondaries.comparison_Aplanatic.aplanat_simulations import trace_options
...
def receiver_position_error_analysis(lfc: AplanatLfc,
                                     site_data: SiteData,
                                     source: RadialSource,
                                     optics: OpticalSettings,
                                     trace: Trace,
...
def receiver_position_error_range_analysis(lfc: AplanatLfc,
                                           site_data: SiteData,
                                           source: RadialSource,
                                           optics: OpticalSettings,
                                           trace: Trace,
    ...
    cm_displacements = linspace(start=-5, stop=5, num=11) * 10

    # mm_displacements = linspace(start=-20, stop=20, num=21)
    # displacement_values = array(list(set(cm_displacements.tolist() + mm_displacements.tolist())))
    # displacement_values.sort()

    displacement_values = cm_displacements
...
lfc_optics = [aplanat_lfc, aplanat_lfc_cpc_red_gap, aplanat_lfc_cec_red_gap]
for lfc_concentrator in lfc_optics:
    dx_data, dy_data = receiver_position_error_range_analysis(lfc=lfc_concentrator,
                                                              site_data=evora,
