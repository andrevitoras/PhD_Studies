from matplotlib.ticker import EngFormatter, StrMethodFormatter, FormatStrFormatter

from on_secondaries.comparison_Aplanatic.aplanat_base_models import *
from on_secondaries.comparison_Aplanatic.aplanat_simulations import simulation_path, trace_options, dni, \
    angle_step, norm_flux_lower_value
...
def plot_metrics(
    ...
    if figure_path is not None:
        plt.savefig(Path(figure_path, f'aplanatic_optical_characterization.{fig_format}'))
    plt.show()
...
def plot_averages_var(optics_to_plot: list, optics_labels: list,
    ...
    plt.tight_layout(pad=0.25, w_pad=0.5, h_pad=0.5)
    if figure_path is not None:
        plt.savefig(Path(figure_path, f'aplanatic_avg_metrics.{fig_format}'))
    plt.show()
...
if __name__ == '__main__':
    ...
    plot_averages_var(optics_to_plot=optics_list, optics_labels=labels, site_data=evora,
